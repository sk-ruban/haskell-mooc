...
prettyPrint size queens = concatMap makeRows [1..size]
  where makeRows r = [getChar (r,c) | c <- [1..size]] ++ "\n"
        getChar coord = if coord `elem` queens then 'Q' else '.'
...
danger candidate queens = any (attackedBy candidate) queens
  where attackedBy coord queen = sameRow coord queen || sameCol coord queen || sameDiag coord queen || sameAntidiag coord queen
...
prettyPrint2 size queens = concatMap makeRows [1..size]
  where makeRows r = [getChar (r,c) | c <- [1..size]] ++ "\n"
        getChar coord
          | coord `elem` queens = 'Q'
          | danger coord queens = '#'
...
fixFirst n ((row,col):rest) =
  if col > n
  then Nothing
  else if not (danger (row,col) rest)
  then Just ((row,col):rest)
  else case find (\pos -> not (danger pos rest)) candidates of
    Just newPos -> Just (newPos:rest)
    Nothing -> Nothing
  where candidates = [(row,c) | c <- [col+1..n]]
...
backtrack (x:(r,c):xs) = nextCol (r,c):xs
