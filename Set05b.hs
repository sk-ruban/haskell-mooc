...
walk (StepL:path) (Node x l r) = walk path l
walk (StepR:path) (Node x l r)= walk path r
...
set path _ Empty = Empty
set [] val (Node _ l r) = Node val l r
set (a:as) val (Node x l r)
    | a == StepL = Node x (set as val l) r
    | otherwise  = Node x l (set as val r)
