...
appendAll ref lis = do
  cur <- readIORef ref
  let new = cur ++ concat lis
  writeIORef ref new
...
hFetchLines h = fmap lines (hGetContents h)
...
hSelectLines h nums = do
  all <- hFetchLines h
  let numbered = zip all [1..]
  let filtered = filter (\(_, index) -> elem index nums) numbered
...
interact' f state = do
  input <- getLine
  let (cont, output, new) =  f (input, state)
  putStrLn output
  if cont
    then interact' f new
    else return new
