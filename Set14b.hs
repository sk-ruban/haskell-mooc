...
deposit conn acc amt = execute conn depositQuery (acc, amt)
...
balance conn acc = do
    amt <- query conn balanceQuery [acc] :: IO [[Int]]
    return $ sum $ concat amt
...
parseCommand path =  case path of
    [cmd, acc] | cmd == T.pack "balance" -> Just (Balance acc)
    [cmd, acc, amt] | cmd == T.pack "deposit" -> case parseInt amt of
        Just amt -> Just (Deposit acc amt)
        Nothing -> Nothing
    [cmd, acc, amt] | cmd == T.pack "withdraw" -> case parseInt amt of
        Just amt -> Just (Withdraw acc amt)
        Nothing -> Nothing
    _ -> Nothing
...
perform conn maybeCmd = case maybeCmd of
    Just (Deposit acc amt) -> do
        deposit conn acc amt
        return $ T.pack "OK"
    Just (Withdraw acc amt) -> do
        deposit conn acc (-1 * amt)
        return $ T.pack "OK"
    Just (Balance acc) -> do
...
server db request respond = do
    ...
    res <- perform db (parseCommand pathSegments)
