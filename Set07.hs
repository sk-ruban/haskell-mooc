...
member n (Set x) = elem n x

-- add a member to a set
add :: (Ord a, Eq a) => a -> Set a -> Set a
add n (Set x) = Set . sort . nub $ (n : x)
...
data State = Start | Error | Finished | HaveEggs | HaveFlour | HaveSugar | HaveBoth | Mixed
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = HaveEggs
step HaveEggs AddFlour = HaveFlour
step HaveEggs AddSugar = HaveSugar
step HaveFlour AddSugar = HaveBoth
step HaveSugar AddFlour = HaveBoth
step HaveBoth Mix = Mixed
...
reverseNonEmpty (x :| xs) = last (x:xs) :| reverse (init (x:xs))
...
passwordAllowed str1 (MinimumLength n) = length str1 >= n
passwordAllowed str1 (ContainsSome str2) = any (`elem` str1) str2
passwordAllowed str1 (DoesNotContain str2) = not $ any (`elem` str1) str2
passwordAllowed str1 (And x y) = passwordAllowed str1 x && passwordAllowed str1 y
passwordAllowed str1 (Or x y) = passwordAllowed str1 x || passwordAllowed str1 y
