...
rectangle x0 y0 w h = Shape f
  where f (Coord x y) = x >= x0 && x < width && y >= y0 && y < height
        height = y0 + h
        width = x0 + w
------------------------------------------------------------------------------
...
instance Transform Flip where
  apply flip (Picture f) = Picture (f . flipCoord flip)
    where flipCoord FlipX (Coord x y) = Coord (-1*x) y
          flipCoord FlipY (Coord x y) = Coord x (-1*y)
          flipCoord FlipXY (Coord x y) = Coord y x
------------------------------------------------------------------------------
...
averageColors colors = Color avgR avgG avgB
  where n = length colors
        avgR = sum [getRed c | c <- colors] `div` n
        avgB = sum [getBlue c | c <- colors] `div` n
        avgG = sum [getGreen c | c <- colors] `div` n

instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where g (Coord x y) = averageColors [f (Coord x y), f (Coord (x+1) y), f (Coord (x-1) y), f (Coord x (y+1)), f (Coord x (y-1))]
------------------------------------------------------------------------------
