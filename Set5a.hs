...
totalPrice (MkShoppingEntry _ cost num) = cost * fromIntegral num
...
buyOneMore (MkShoppingEntry name cost num) = MkShoppingEntry name cost (num+1)
...
fromList [] = Empty
fromList ((k,v):x) = Pair k v (fromList x)
...
fromBinHelper End pos = 0
fromBinHelper (O bin) pos = fromBinHelper bin (pos+1)
fromBinHelper (I bin) pos = 2^pos + fromBinHelper bin (pos+1)
