...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode val rest) = LNode (f val) (fmap f rest)
...
count x lis = foldr (\y acc -> if y == x then acc + 1 else acc) 0 lis
...
inBoth x y = filter (\x -> x `elem` (toList y)) (toList x)
...
instance Foldable List where
  foldr f z Empty = z
  foldr f z (LNode val rest) = f val (foldr f z rest)
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node val l r) = sumTree l <> val <> sumTree r
