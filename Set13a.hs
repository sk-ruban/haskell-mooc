...
checkNumber (x, y) = if any isDigit x || any isDigit y
  then Nothing
  else Just (x, y)
...
checkCapitals (x:xs, y:ys) = if isUpper x && isUpper y
  then Just (x:xs, y:ys)
  else Nothing
...
countAndLog pred (x:xs) = case pred x of
  True -> do
    msg (show x)
    rest <- countAndLog pred xs
    return (1 + rest)
  False -> countAndLog pred xs
...
-- operation shouldn't change the state o
-- f the Bank. The functions
...
balance accountName = BankOp helper
  where
    helper (Bank accounts) = (maybe 0 id (Map.lookup accountName accounts), Bank accounts)
...
count x = do
  lis <- get
  case lookup x lis of
    Just n -> put (map (\(y, count) -> if y == x then (x, count+1) else (y, count)) lis)
    Nothing -> put ((x,1):lis)
