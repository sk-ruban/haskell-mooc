...
isSorted xs = property (all (\(a,b) -> a <= b) (zip xs (tail xs)))
...
frequenciesProp freq input =
    let realInput = getNonEmpty input
        output = freq realInput
    in conjoin [
        sumIsLength realInput output,
        inputInOutput realInput output,
        outputInInput realInput output
    ]
