  ...
  if place `elem` visited
    then return ()
      ...
      case lookup place maze of
        Just neigbours -> mapM_ (visit maze) neigbours
        Nothing -> return ()
...
instance Functor SL where
  -- implement fmap
  fmap f (SL computation) = SL (\s ->
    let (res, newState, messages) = computation s
    in (f res, newState, messages))
...
instance Monad SL where
  ...
  return x = SL (\s -> (x, s, []))
  (SL comp1) >>= f = SL (\s ->
        let (res1, state1, messages1) = comp1 s
            SL comp2 = f res1
            (res2, state2, messages2) = comp2 state1
        in (res2, state2, messages1 ++ messages2))
