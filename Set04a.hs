...
rangeOf lis = maximum lis - minimum lis
...
longest :: Ord a => [[a]] -> [a]
longest (x:xs) = foldr chooseLong x xs
    where chooseLong lis1 lis2
            | length lis1 > length lis2 = lis1
            | length lis1 < length lis2 = lis2
            | head lis1 < head lis2 = lis1
            | otherwise = lis2
...
incrementKey k ((key, val):xs)
    | k == key  = (key, (val + 1)): incrementKey k xs
    | otherwise = (key, val) : incrementKey k xs
...
freqs xs = foldr addCount Map.empty xs
    where addCount x = Map.alter increment x
...
transfer from to amount bank  
    ...
    | otherwise                  = 
        ...
        (Map.insert to (toBalance + amount) bank)
    where fromBalance = bank Map.! from
          toBalance   = bank Map.! to
...
maxIndex arr = snd $ maximum [(v, i) | (i, v) <- assocs arr]
