...
calculator op num = parseOp op  <*> readMaybe num
    where parseOp s = lookup s [("negate", negate), ("double", (*2))]
...
validateAddress streetName streetNumber postCode = Address <$> validateStreet streetName <*> validateNumber streetNumber <*> validatePostcode postCode
    where validateStreet name = check ((length name) <= 20) "Invalid street name" name
          validateNumber num = check (all isDigit num) "Invalid street number" num
          validatePostcode code = check (((length code)) == 5 && all isDigit code) "Invalid postcode" code
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
    liftA2 (\x y -> [x,y]) person1 person2
    where
...
boolOrInt s = tryBool s <|> tryInt s
    where
        tryBool s = case readMaybe s of
            Just b -> pure (Left b)
            Nothing -> invalid "Not a Bool"
        tryInt s = case readMaybe s of
            Just b -> pure (Right b)
...
normalizePhone str = lengthCheck *> traverse charCheck stripped
    where
        stripped = filter (/= ' ') str
        lengthCheck = check ((length stripped) <= 10) "Too long" ()
        charCheck c = check (isDigit c) ("Invalid character: " ++ [c]) c
...
parseExpression input =
    case words input of
        [left, op, right] -> buildExp <$> parseArg left <*> parseOp op <*> parseArg right
        _ -> invalid ("Invalid expression: " ++ input)
        where
            buildExp leftArg operator rightArg = operator leftArg rightArg
            parseArg s = parseNumber s <|> parseVariable s
            parseOp s = case s of
                "+" -> pure Plus
                "-" -> pure Minus
                _ -> invalid ("Unknown operator: " ++ s)
            parseNumber s = case readMaybe s of
                Just b -> pure (Number b)
                Nothing -> invalid ("Invalid number: " ++ s)
            parseVariable s = case s of
                [c] | isAlpha c -> pure (Variable c)
                _ -> invalid ("Invalid variable: " ++ s)
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap h (Both nested) = Both (fmap (fmap h) nested)
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  liftA2 h (Both nested1) (Both nested2) = Both (liftA2 (liftA2 h) nested1 nested2)
