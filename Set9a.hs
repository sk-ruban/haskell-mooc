...
countValid notes = sum (map count notes)
  where count note
          | (note !! 2 == note !! 4) || (note !! 3 == note !! 5) = 1
          | otherwise = 0
...
sumSuccess lis
  | vals == [] = Left "no data"
  | otherwise = Right (sum vals)
  where vals = rights lis         -- import Data.Either
...
lock (Opened pw) = Locked pw

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode code (Opened pw) = Opened code
changeCode code (Locked pw) = Locked pw
...
instance Eq Text where
  Text str1 == Text str2 = filter (not . isSpace) str1 == filter (not . isSpace) str2
...
compose arr1 arr2 = [(k,v2) | (k,v1) <- arr1, Just v2 <- [lookup v1 arr2]]
...
permute indices input = map snd (sortOn fst (zip indices input))
