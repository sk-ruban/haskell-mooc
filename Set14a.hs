...
greetText phrase = case (T.length phrase) > 15 of
    True -> (T.pack "Hello, ") <> T.take 15 phrase <> (T.pack "...!")
    False -> (T.pack "Hello, ") <> phrase <> (T.pack "!")
...
shout :: T.Text -> T.Text
shout text =
    let indexed = zip [1..] (T.words text)
        processed = map (\(index, word) -> if odd index then T.toUpper word else word) indexed
        in T.unwords processed
...
takeStrict n text = TL.toStrict (TL.take (fromIntegral n) text)
...
byteRange byte = case B.unpack byte of
    [] -> 0
    bytes -> maximum bytes - minimum bytes
...
xorChecksum byte = case B.unpack byte of
    [] -> 0
    bytes -> foldr xor 0 bytes
...
countUtf8Chars text = case decodeUtf8' text of
    Left error ->  Nothing
    Right valid -> Just (T.length valid)
...
pingpong text = BL.fromChunks (cycle [text <> B.reverse text])
